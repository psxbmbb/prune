...
class EDSR_T(Model):
    ...
    def prune(self,batch_size,sess):
        rec_f = open('rec2.txt','a')
        grad_sum = []
        for tmp in range(19):
            grad_sum.append([])
            if(tmp<16):
        ...
        for index in range(19):
            if(index>=16):
                ...
                candidate.append(aim[index].argsort()[0:self.prunesize])
                candidate[index] = sorted(candidate[index])
            print(aim[index][candidate[index]])
        print("start_pruning")

        print(candidate)
        rec_f.write(str(candidate))
        rec_f.write("\n")
        ...
        for index,element in enumerate(candidate):
            ...
            sess.run(tf.assign(w_next, w_next_new))
        rec_f.close()
    def train(self,batch_size= 10, iterations=1000, lr_init=1e-4, lr_decay=0.5, decay_every=2e5,
        ...
        with self.sess as sess:
            ...
            for i in tqdm(range(iterations)):
                ...
                if(i!=0 and i%20000==3000):
                    self.prune(batch_size,sess)
