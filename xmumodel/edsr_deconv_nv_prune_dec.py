...
class EDSR_D(Model):
    def buildModel(self):
        ...
        x = tl.Conv2d(x, self.feature_size-self.prunedlist[17], [3, 3], act=tf.nn.relu, name='deconv_conv_1')
        x = TransposedConv2dLayer(x, self.feature_size, [5, 5], [2, 2], name='deconv_2')
        if self.scale==8:
            x = tl.Conv2d(x, self.feature_size-self.prunedlist[18], [3, 3], act=tf.nn.relu, name='deconv_conv_2')
	...
	#self.train_op = optimizer.minimize(self.loss)
        ...
        with self.sess as sess:
            ...
            grad_sum = np.zeros(shape=(19,3,3,self.feature_size, self.feature_size-self.prunedlist[0]))
            ...
            for index,element in enumerate(candidate):
                ...
                if(index == 16):
                    var_tensor = tl.get_variables_with_name("m1")
                    w_next = deconv_var[0]
                if(index == 17):
                ...
                if(index>=16):
                    w_next_np = np.delete(w_next_np, element, -1)
                else:
